...
This behaviour can be changed to more closely approximate the old behaviour by setting `rootDirFileName` to `""` (an empty string). In that case, the project root directory is determined instead by looking for a workspace root directory that contains the current file. If the current file is not contained in a workspace root directory then it uses the same directory as the edited file. If multiple workspace root directories contain the current file (in the case of nested workspace root directories, for example) the project root will be determined by whichever one is found first.
